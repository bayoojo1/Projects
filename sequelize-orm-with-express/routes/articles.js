...
router.get("/:author/filter", asyncHandler(async(req, res) => {
  const articles = await Article.findAll({ where: {author: req.params.author}});
  if(articles) {
    res.render("articles/index", { articles, title: "Authored by - " + req.params.author });
  } else {
    res.sendStatus(404);
  }
}));
